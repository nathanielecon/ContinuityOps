...
- **Fix:** Add `terraform/ci-bootstrap/` + CloudShell
  ...
  (plan/apply, role `continuityops-gha`, account `000000000000`, roots under
...
- **Target account (not a secret):** `000000000000` / root `arn:aws:iam::000000000000:root`. ContinuityOps CI role: `arn:aws:iam::000000000000:role/continuityops-gha`. Env string only: `.../role/CursorCloudAgent` (unused).
- **Fix / control (workarounds in order):**
  ...
  2. **Proven — local bottleneck:** laptop `aws login` as account `000000000000`; orchestrator runs apply/evidence locally; Cloud seat stays repo-only.
...
## 2026-09-06 — Rights-pending corpus moved to private successor

The mixed QTI/PDF workstream and its corpus-specific break/fix entries moved to
the private `nathanielecon/math-corrections-qti` repository. They were removed
from ContinuityOps history after private validation and encrypted mirror
verification. This log intentionally retains only ContinuityOps platform
entries; see [`docs/QTI_MIGRATION.md`](docs/QTI_MIGRATION.md) for the public
scope boundary.
