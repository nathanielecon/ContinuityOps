...
\blockhead{Question}
\noindent $2.4 \times 1.5 = $\ansblank\hint{decimal}
...
\blockhead{Question}
\noindent $5.6 \div 0.8 = $\ansblank\hint{decimal}
...
\blockhead{Question}
\noindent Greater: $\dfrac{5}{8}$ or $0.61$?\ \blank[2.4cm]
...
\begin{steps}
  ...
  \item $8$ does not go into $5$, so write $0.$ and bring down a $0$ to make $50$.
  \item $8$ into $50$ is $6$ (because $8 \times 6 = 48$), remainder $2$.
  \item Bring down $0$: $20 \div 8 = 2$ (because $8 \times 2 = 16$), remainder $4$.
  \item Bring down $0$: $40 \div 8 = 5$. The digits $6$, $2$, $5$ fall after the
        decimal point, so $5 \div 8 = 0.625$.
...
\blockhead{Question}
\[ \frac{4}{7} = \frac{?}{21} \qquad {?} = \underline{\hspace{2.2cm}} \]
...
\blockhead{Question}
\[ \frac{5}{6} = \frac{x}{18} \qquad x = \underline{\hspace{2.2cm}} \]
...
\blockhead{Question}
\noindent In $7m$, $7$ is the \blank[3.2cm]
...
\blockhead{Question}
\noindent Undo $+8$, use inverse op: \blank[2.4cm]
...
\blockhead{Question}
\noindent Undo $\times 5$, use inverse op: \blank[2.4cm]

\checked{divide by $5$ (or $\div 5$)}

\blockhead{What the question is asking}
\noindent This question asks which operation undoes multiplying by $5$.
...
\blockhead{Work the problem one tiny step at a time}
\begin{steps}
  \item Starting operation: multiply by $5$ (written $\times 5$).
  \item The inverse of multiply is divide.
  \item To undo multiplying by $5$, divide by $5$ (written $\div 5$).
  \item Numeric round-trip: start with $4$, multiply by $5$ to get $20$, then divide by
        $5$ to get back to $4$.
\end{steps}

\finalans{divide by $5$ (or $\div 5$)}

\blockhead{Why this makes sense}
\noindent Multiply and divide cancel (they undo each other, same idea as F3).
The inverse of $\times 5$ is $\div 5$.

% ---------------------------------------------------------------------------
\question{F5}

\blockhead{Question}
\noindent Combine like terms: $6y - 2y = $\ansblank
...
\blockhead{Question}
\noindent If $n = 4$, then $5n - 3 = $\ansblank
