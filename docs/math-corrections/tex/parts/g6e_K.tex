...
\blockhead{Question}
\noindent $700.32 - 84.67 = $\ansblank\hint{decimal}

\checked{$615.65$}

\blockhead{What the question is asking}
\noindent This question asks you to subtract $84.67$ from $700.32$ and write the
...
\blockhead{Work the problem one tiny step at a time}
\begin{steps}
  \item Both numbers already show two decimal places, so nothing needs padding:
        $700.32$ and $84.67$.
  \item Stack $700.32$ above $84.67$ with the decimal points lined up (see the
        grid below). $84.67$ has no hundreds digit, so its $8$ sits in the tens
        column.
  \item Break $84.67$ into parts: $80 + 4 + 0.67$. Subtract one part at a time
        from $700.32$.
  \item Subtract the tens: $700.32 - 80 = 620.32$.
  \item Subtract the ones: $620.32 - 4 = 616.32$.
  \item Subtract six tenths: $616.32 - 0.60 = 615.72$.
  \item Subtract seven hundredths: $615.72 - 0.07 = 615.65$.
  \item So the difference is $615.65$.
\end{steps}
...
$615.65 + 84.67$. Hundredths: $5 + 7 = 12$ (write $2$, carry $1$).
Tenths: $6 + 6 + 1 = 13$ (write $3$, carry $1$).
Ones: $5 + 4 + 1 = 10$ (write $0$, carry $1$).
Tens: $1 + 8 + 1 = 10$ (write $0$, carry $1$).
Hundreds: $6 + 0 + 1 = 7$. Result $700.32$, which matches the starting number.
Check passes.
...
\blockhead{Question}
\noindent $7.2 \div 0.8 = $\ansblank\hint{decimal}
...
\blockhead{Question}
\noindent $\dfrac{1}{2} + \dfrac{1}{4} = $\ansblank\hint{simplest}
...
\blockhead{Question}
\noindent $\dfrac{5}{6} - \dfrac{1}{3} = $\ansblank\hint{simplest}
...
\blockhead{Question}
\noindent $\dfrac{5}{8} \times \dfrac{4}{15} = $\ansblank\hint{simplest}
...
\blockhead{Question}
\noindent $\dfrac{7}{8} \div \dfrac{1}{4} = $\ansblank\hint{simplest}
...
\blockhead{Question}
\noindent Commutative: $7 \times a = $\ansblank

\checked{$a \times 7$}

\blockhead{What the question is asking}
\noindent This question asks you to rewrite $7 \times a$ by swapping the order of
the two factors being multiplied (order can change; the answer stays the same).
...
\begin{steps}
  ...
  \item After the switch, first factor is $a$, second factor is $7$: write
        $a \times 7$.
  \item Numeric check with a number for $a$: if $a = 3$, then $7 \times 3 = 21$
        and $3 \times 7 = 21$. Same product.
\end{steps}
...
\blockhead{Question}
\noindent Associative: $(x + 5) + 8 = $\ansblank
...
\blockhead{Question}
\noindent Is subtraction associative? Compare $(12 - 4) - 3$ and
$12 - (4 - 3)$:\ansblank
