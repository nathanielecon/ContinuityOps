...
\begin{tikzpicture}[y=0.011cm,x=1cm]
  ...
  % ticks and labels
  % Ticks may live in the loop; the LABELS must not. TikZ scopes each \foreach
  % iteration in its own group, so a node named inside the loop is invisible to
  % the \draw calls below and the anchor silently falls back to a stale
  % coordinate -- which compiles clean while keeping the old geometry.
  \foreach \y in {450,225,0,-120,-200}{\draw (-0.08,\y) -- (0.08,\y);}
  \node[right=2pt,font=\footnotesize,inner xsep=2pt] (labyPeak)   at (0.08,450)  {$450$};
  \node[right=2pt,font=\footnotesize,inner xsep=2pt] (labyMid)    at (0.08,225)  {$225$};
  \node[right=2pt,font=\footnotesize,inner xsep=2pt] (labyZero)   at (0.08,0)    {sea level: $0$};
  \node[right=2pt,font=\footnotesize,inner xsep=2pt] (labyValley) at (0.08,-120) {$-120$};
  \node[right=2pt,font=\footnotesize,inner xsep=2pt] (labyLow)    at (0.08,-200) {$-200$};
  % marked points
