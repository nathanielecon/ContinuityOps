...
\begin{words}
  ...
  \item A \term{signed number}uses a plus or minus sign to show direction:
...
\blockhead{Question}
\noindent On a number line, zero is located in the middle. We have two mystery numbers, $a$ and $b$, where $a < 0$, $b > 0$,
and $a + b = 0$. Write an equation about the absolute values of $a$ and $b$
that must be true.
...
\begin{words}
  ...
  \item \term{Absolute value}of a number, written $|x|$, is the distance from that number to zero.
...
\begin{words}
  ...
  \item \term{Distance}tells how far apart two points are. Distance is never negative.
  \item \term{Absolute value}$|x|$ means distance from zero on the number line. Distance is
        never negative.
  \item An \term{integer}is one of the numbers $\ldots, -2, -1, 0, 1, 2, \ldots$; it has no decimal part.
...
\blockhead{What the question is asking}
\noindent This question asks you to change $\tfrac{3}{8}$ into a number that
uses a dot for parts of a whole.

\blockhead{Important words}
\begin{words}
  \item A \term{fraction}$\tfrac{3}{8}$ means $3$ parts out of $8$ equal parts. The top number is
...
\blockhead{Work the problem one tiny step at a time}
\begin{steps}
  \item Write the fraction: $\tfrac{3}{8}$.
...
\checked{Jordan; $0.\overline{27} = \tfrac{3}{11}$.}

\blockhead{What the question is asking}
\noindent This question asks you to decide whether a repeating decimal is rational and prove it.

\blockhead{Important words}
\begin{words}
  \item A \term{repeating decimal}has a digit pattern that repeats forever. The bar notation $0.\overline{27}$ means the digits $27$ repeat: $0.272727\ldots$.
  \item A \term{rational number}can be written as a fraction of integers with a nonzero denominator.
\end{words}
...
\begin{steps}
  ...
  \item Subtract the first equation from the second: $100x - x = 27.\overline{27} - 0.\overline{27}$, so $99x = 27$.
  \item Divide by $99$: $x = \tfrac{27}{99} = \tfrac{3}{11}$.
\end{steps}

\finalans{Jordan is correct; $0.\overline{27} = \tfrac{3}{11}$.}
