...
**Amended after three rounds — the author ruled to keep widening.** The rate was
measured and put to the author: roughly two new defensible terms per round,
linear rather than decaying. The ruling is unchanged in principle and now
explicit in practice — **when a term surfaces, add it.** Do not propose closing
the stem, and do not treat the open set as a defect to be designed away.

Terms accepted to date:

- `g6_s1_f1` — `coefficient`, `numerical coefficient`, `factor`,
  `numerical factor`, `multiplier`, `numerical multiplier`, `constant`,
  `constant factor`
- `g6_s1_f2` — `variable`, `unknown`, `unknown number`, `unknown value`,
  `unknown quantity`, `unknown variable`, `missing number`, `missing addend`,
  `missing value`

**If you find another defensible rejected term: report it, and expect it to be
added.** Reporting remains the job. The author wants the recurrence visible even
while accepting it.
