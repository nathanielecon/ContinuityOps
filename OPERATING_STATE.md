...
| D-047 | **Zero-hop mechanical pipeline + pager** (owner directive 2026-07-18): routine phase hops are evented by GHA `.github/workflows/pipeline-zero-hop.yml` with no junior timer — publish-ok→D-037, verdict pass+CI→merge candidate, verdict fail→fixer, judge `merge_ready: no`→nixer (path B), nixer→fixer, fixer land→provisional judges, provisional pass→fresh council, fresh pass+CI→main, publish-failed one re-nudge, keep-warm smoke. Glow: fail→auto-nixer→fixer→candidate→provisional loop until pass→fresh council→main only on fresh pass. Junior is exception-only (scope fights, fuzzy stop/escalate, two-strike halt, cross-lane when locks exist, routing packets). Pager: `<!-- continuityops-chief-page-v1 -->` + label `chief-pager` (sticky chief issue); junior exceptions via `JR-EXCEPTION-*` `@codex` wake. Candidate branch `candidate/portfolio-<7sha>`; main only after fresh pass. Amends practical reading of D-041/D-042 (junior no longer owns routine hop timing). | accepted | Zero hop latency between mechanical jobs; junior/chief paged only on exceptions |
