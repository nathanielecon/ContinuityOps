...
```json
{
  ...
  "next_actions": [
    ...
    "Keep-warm: cloud-native issue #4 warm stamp; at dispatch, if the latest Codex task timestamp is older than ~10h, post a trivial @codex smoke first and stamp on its reply; supervisor heartbeat smokes at >9h; local Invoke-CodexCloudWarm.ps1 is fallback-lane only",
